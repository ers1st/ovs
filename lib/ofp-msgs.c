...
ofpraw_pull(enum ofpraw *rawp, struct ofpbuf *msg)
{
    ...
    *rawp = 0;
    ...
    fprintf(f, "---------------------------------------\n");
    fprintf(f, "(ofpraw_pull) raw = %d\n", raw);
    fprintf(f, "(ofpraw_pull) instance->hdrs_len = %d\n", instance->hdrs_len);
    fprintf(f, "(ofpraw_pull) info->min_body = %d\n", info->min_body);
    fprintf(f, "(ofpraw_pull) len = %d\n", len);
    fprintf(f, "(ofpraw_pull) min_len = %d\n", min_len);
    fprintf(f, "---------------------------------------\n");
    fclose(f);
...
ofptype_decode(enum ofptype *typep, const struct ofp_header *oh)
{
    ...
    *typep = error ? 0 : ofptype_from_ofpraw(raw);
    if (error) {
    	FILE *f = fopen("/home/davide/Scrivania/state_mod", "a");
    	fprintf(f, "Errore in ofpraw_decode()\n");
    	fclose(f);
    }
    return error;
}
